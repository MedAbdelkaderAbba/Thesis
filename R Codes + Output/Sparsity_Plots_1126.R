...
out_mp <- out_mp/p
names(out_mp)<- c("Lasso" , "Horseshoe" , "Bayes SQL" , "Sqrt-DL")
...
library(ggplot2)
 ...
 misclass<- misclas + scale_x_continuous(breaks = prop)
 
 misclass <- misclass+theme_bw()+theme(strip.text.y = element_text(size=15, face="bold"))+
   theme(panel.grid.major = element_blank(),panel.grid.minor = element_blank(), axis.line = element_line(colour = "black"))
 misclass <- misclass + theme(axis.title.y = element_text(size = rel(1.2), angle = 90))+
   theme(axis.title.x = element_text(size = rel(1.2)))
 misclass <- misclass + theme(legend.position = c(0.2,0.8),legend.key = element_rect(colour = "black"),legend.text=element_text(size=rel(1.2)))
 misclass<- misclass+ theme(axis.text = element_text(size = rel(1.2)))
 print(misclass)
 
 ggsave(file = "sparsity_adapt_n=p.pdf", width = 5)
